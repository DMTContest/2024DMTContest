...
public class JG3Instance : MonoBehaviour
{
    private static JG3Instance instance = null;
    ...
    private void Update()
    {
        ...
        if (SceneManager.GetActiveScene().name == "TuxingKuaida")
        {
            _time = GetTimeFromScene1();
        }
        if (SceneManager.GetActiveScene().name == "Kuaida2")
        {
    ...
    {
        TXJG32Time s2 = FindObjectOfType<TXJG32Time>();
        if (s2 != null)
        {
            return s2.timeStart;
        }
